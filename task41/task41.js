var calendar = (function(){
	...
	_calendar.prototype = {
		...
		init:function(){   
			...
			this.container.css({'width':'414px','height':'441px'});

 			this.creatHeader();

             var self = this;
			//日历外框
			this.mainEle = $("<div class='calendar'></div>").appendTo(this.container).css({'width':'350px','height':'380px','border':'1px solid black','clear':'both','margin-left':'62px','position':'relative'});


			var triangle = $('<div>').css({'width':'0','height':'0','border-bottom':'14px solid red','border-left':'7px solid transparent','border-right':'7px solid transparent','position':'absolute','top':'-14px','left':'20px'}).appendTo(this.mainEle);
        ...
        creatHeader:function(){
        	var self = this;
        	var ospan =  $('<span>').addClass('icoin').css({'width':'60px','height':'50px','backgroundColor':'lightgrey','text-align':'center','border':'1px solid grey','border-radius':'5px','display':'inline-block','margin-top':'2px','float':'left'}).appendTo(this.container).click(function(){self.togCalendar()});
        		
        		$('<img>').css({'width':'40px','height':'40px'}).attr('src','../image/日历.png').appendTo(ospan);
        	
        	this.oinput = $('<input>').css({'width':'285px','height':'50px','border':'1px solid grey','border-radius':'5px','float':'left','margin-bottom':'5px','font-size':'28px'}).appendTo(this.container).click(function(){self.togCalendar()});
       ...
       /* getLastDate:function(date){
        ...
        showInput:function(date){
 				var d = date.getDate()<10?'0'+date.getDate():date.getDate(),
				m = date.getMonth()+1<10?'0'+(date.getMonth()+1):date.getMonth()+1,
				y = date.getFullYear(),
				temp = m + "/" + d + "/" + y;
				this.oinput.attr('value',temp);
				this.togCalendar();				
        },
